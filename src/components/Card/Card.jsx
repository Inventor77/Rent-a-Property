	...
	return (
		<Box sx={{ position: "relative" }}>
			<Card sx={{ width: 384, maxWidth: 384, borderRadius: "10px" }}>
				...
				<SideTag>
					<AutoAwesomeIcon
						sx={{ color: "#FFFFFF", mr: "8px", fontSize: "medium" }}
					/>{" "}
					Popular
				</SideTag>
				<CardContent>
					<Grid
						...
						<Grid container spacing={0} direction='column' xs>
							<Grid>
								<Typography className='card_price'>
									{`$${data?.price}`}
									<span>/month</span>
								</Typography>
							</Grid>
				...
				<CardActions>
					<Grid container spacing={0}>
						<Grid xs>
							<Item elevation={0}>
								<BedOutlinedIcon sx={{ color: "#6F65EF", mr: "8px" }} />{" "}
								{`${data?.beds} Beds`}
							</Item>
						</Grid>
						<Grid xs>
							<Item elevation={0}>
								<BathtubOutlinedIcon sx={{ color: "#6F65EF", mr: "8px" }} />{" "}
								{`${data?.bathrooms} Bathrooms`}
							</Item>
						</Grid>
						<Grid xs>
							<Item elevation={0}>
								<GrassIcon sx={{ color: "#6F65EF", mr: "8px" }} />{" "}
								{`${data?.area}m`}
								<sup>2</sup>
							</Item>
						</Grid>
